...
public class PostService {
    ...
    public void 게시글쓰기(PostWriteReqDto postWriteReqDto) {
        ...
        // 2번 이미지 파일명을 Post 오브젝트에 thunmail 옮겨야 함
        // 3번 title, content도 Post 오브젝트에 옮기고
        // 4번 UserId도 Post오브젝트에 옮기기
        // 5번 categoryId 도 post 오브젝트에 옮기기
        // 6번 save하면 끝
    }

    public PostRespDto 게시글목록보기(Integer userId) {
        List<Post> postEntity = postRepository.findByUserId(userId);
        List<Category> categoriesEntity = categoryRepository.findByUserId(userId);
        PostRespDto postRespDto = new PostRespDto(postEntity, categoriesEntity);
        return postRespDto;
    }
}